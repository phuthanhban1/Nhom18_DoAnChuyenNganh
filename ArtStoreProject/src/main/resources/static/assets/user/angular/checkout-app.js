...
function handlePayment() {
    var paymentMethod = $("input[name='shipping_method']:checked").val();

    if (paymentMethod === "0") {
        transferConfirm(); // Chuyển đến bước xác nhận đơn hàng
...
//        ward: ward,
    ...
    // Gửi thông tin đơn hàng đến server để xử lý thanh toán
    $.ajax({
        ...
        success: function(response) {
            // Xử lý phản hồi từ server, ví dụ: chuyển hướng đến trang thanh toán của cổng thanh toán
            window.location.href = response.paymentUrl;
        },
        error: function(error) {
            console.error('Error processing online payment', error);
            alert('Có lỗi xảy ra khi xử lý thanh toán online. Vui lòng thử lại.');
        }
    });
}