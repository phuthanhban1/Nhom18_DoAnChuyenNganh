...
import haui.store.dao.DiscountDao;
import haui.store.dao.OrderDao;
import haui.store.dao.ProductDao;
...
public class OrderServiceImpl implements OrderService{
	...
	public void cancelOrder(String id) {
		...
		for (Order list : listOrder) {
			...
			product.setQuantity(product.getQuantity() + list.getQuality());
			list.setStatus("3");
			orderDao.save(list);
	...
	public StatisticalTotalOrder getStatisticalTotalOrderOnDay(int day, int month, int year) {
		...
		long success = orderSuccess.size();
		long wait = orderWait.size();
		long transport = orderTransport.size();
		long cancel = orderCancel.size();
	...
	public void updateOrderStatus(String orderId, String status) {
		// Logic to update order status based on the payment status
		Order order = orderDao.findById(Integer.parseInt(orderId)).orElse(null);
		if (order != null) {
